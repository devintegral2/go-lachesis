...
func TestFlushableParallelXXX(t *testing.T) {
	...
	t.Logf("not flushed pairs count:%v", flushableDb.NotFlushedPairs())
...
func TestFlushableParallelV2(t *testing.T) {
	testDuration := 2 * time.Second
	testPairsNum := uint64(1000)
	...
	_ = flushableDb.Flush() // if you remove flushableDb.Flush() from here, then the iterator is failed in the goroutine
	...
	go func() {
		...
		for {
			...
			default:
				...
				assertar.Equal(int64(testPairsNum), int64(i))
				_ = flushableDb.Flush()
			}
		}
